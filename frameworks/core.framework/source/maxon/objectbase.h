...
public:
	...
	{
		Construct<C>();
		static_assert(std::is_same<C, typename C::ComponentClass>::value, "Component class C has to match the first template parameter of its base class Component<C, ...>.");
...
public:
	...
	/// @see AddComponent
	/// @MAXON_ANNOTATION{methodId="AddComponents@142f66f134149341"->"AddComponents@047f2bbac2b714b0"}
	//----------------------------------------------------------------------------------------
	...
	/// @MAXON_ANNOTATION{methodId="Create@f9011904227f5423"->"CreateRef@c2526310a4ba9d20"}
	//----------------------------------------------------------------------------------------
...
{
	template <typename I> using FindInterface = typename std::conditional<std::is_same<I, INTERFACE>::value, CombinedMTableBase, typename SUPER::template FindInterface<I>>::type;
...
template <typename INTERFACE, typename SUPER> struct CombinedMTable : public CombinedMTableBase<INTERFACE, SUPER, !std::is_void<typename SUPER::template FindInterface<INTERFACE>>::value>
{
};
...
#define PRIVATE_MAXON_COMPBASE_BODY(CLS) \
	...
	template <typename S> struct Wrapper : public Interface::Hxx2::template Wrapper<typename SUPER::template Wrapper<S>> \
	{ \
...
public:
	...
	{
		return reinterpret_cast<const maxon::Class<typename INTERFACE::Hxx1::ReferenceClass>&>(_classInfo->_class);
	}
...
protected:
	...
	void RemoveReference() const
	{
		if (MAXON_UNLIKELY(System::GetReferenceCounter(this).Dec() == true))
			System::FreeWithDestructor(const_cast<ObjectInterface*>(this), &DestructInstance);
	}

private:
	static void DestructInstance(ObjectInterface* o);
...
/// @markDeprecated Use ObjectInterface 
...
#define PRIVATE_MAXON_COMPONENT_REGISTER(C, TEMPLATE, TYPENAME, FLAGS, REGISTRY, ...) \
	...
	TEMPLATE const maxon::ComponentDescriptor C::_descriptor((TYPENAME C*) nullptr, PRIVATE_MAXON_COMPONENT_ID(__VA_ARGS__), &maxon::g_translationUnit, __FILE__, (FLAGS) PRIVATE_MAXON_COMPONENT_EXTRA_FLAGS, (REGISTRY*) nullptr)
...
public:
	...
	{
		if (SIZEOF(C) == (std::is_empty<ComponentRoot>::value ? SIZEOF(void*) : SIZEOF(ComponentRoot) + SIZEOF(void*)))
		{
			return OK;
		}
		else if (std::is_copy_assignable<C>::value)
		{
			using AssignType = typename std::conditional<std::is_copy_assignable<C>::value, C, ComponentRoot>::type;
...
#define MAXON_STATIC_REGISTER(C, ...) \
	static maxon::NonvirtualInterfaceImplementation C##_implementation((maxon::StaticHelper<C>*) nullptr, #C, C::PrivateInterface::_staticInterface, (maxon::NonvirtualInterfaceReference::MTableInitializer) &C::PrivateInterface::Hxx2::StaticMTable::Init<C::PrivateInterface::Hxx2::Wrapper<maxon::StaticHelper<C>>>, C::PrivateInterface::REFERENCE_KIND | maxon::EntityBase::FLAGS(__VA_ARGS__), &maxon::g_translationUnit, __FILE__)
...
public:
	...
	ComponentWithBase& operator =(const typename std::conditional<std::is_copy_assignable<PrivateSuper>::value, ComponentWithBase, DummyParamType>::type& src)
	{
...
#define PRIVATE_MAXON_INTERFACE_REGISTER_VIRTUAL(I, POSTFIX, TARGS, TEMPL, IID, NAME, ...) \
	...
	TEMPL maxon::InterfaceReference I::_interface(IID, NAME, g_baseInterfaces_##POSTFIX TARGS, I::MTable::_ids, PRIVATE_MAXON_INITIALIZED_TRANSLATION_UNIT(TEMPL), I::REFERENCE_KIND | I::INTERFACE_FLAGS)
...
#define PRIVATE_MAXON_INTERFACE_REGISTER_VIRTUAL_SINGLE_BASE(I, TEMPL, IID, NAME, BASE) \
	TEMPL maxon::InterfaceReference I::_interface(IID, NAME, BASE, I::MTable::_ids, PRIVATE_MAXON_INITIALIZED_TRANSLATION_UNIT(TEMPL), I::REFERENCE_KIND | I::INTERFACE_FLAGS)
